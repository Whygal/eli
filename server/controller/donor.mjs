...
const getAllDonorsForExcel = async (req, res) => {
  try {
    const donors = await DonorModel.find().populate("group").sort({ date: -1 });

    const jsonData = donors.map((donor) => ({
      name: donor.name,
      amount: donor.amount,
      group: donor.group.name,
      groupHebrew: donor.group.nameHebrew,
      date: donor.date,
    ...
    res.status(500).json({ error: "Failed to fetch and export donors" });
  }
};
